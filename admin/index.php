    ...
    <script>
        ...
        async function loadDashboard() {
            try {
                ...
                if (result.success && result.data) {
                    ...
                    document.getElementById('stat-pedidos').textContent = data.pedidos_hoje || 0;
                    document.getElementById('stat-faturamento').textContent = 
                        `R$ ${(data.faturamento_mensal || 0).toFixed(2).replace('.', ',')}`;
                    document.getElementById('stat-clientes').textContent = data.clientes_ativos || 0;
                    document.getElementById('stat-falta').textContent = data.produtos_falta || 0;
                    
                    // Atualizar badges
                    document.getElementById('badge-produtos').textContent = data.total_produtos || 0;
                    document.getElementById('badge-pedidos').textContent = data.pedidos_hoje || 0;
                    ...
                    if (data.ultimos_pedidos && data.ultimos_pedidos.length > 0) {
                        ...
                        data.ultimos_pedidos.forEach(pedido => {
                            ...
                            tbody.innerHTML += `
                                <tr>
                                    <td><strong>#${String(pedido.id_pedido).padStart(3, '0')}</strong></td>
                        ...
                        data.mais_vendidos.forEach(produto => {
                            tbodyVendidos.innerHTML += `
                                <tr>
                                    ...
                                    <td>R$ ${(produto.receita || 0).toFixed(2).replace('.', ',')}</td>
                                </tr>
                            `;
                        });
                    } else {
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    result.data.forEach(produto => {
                        ...
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>${produto.sku || '---'}</td>
                                <td>R$ ${(produto.preco_venda || 0).toFixed(2).replace('.', ',')}</td>
                    ...
                    document.getElementById('tabela-produtos').innerHTML = `
                        <tr>
                            <td colspan="8" style="text-align: center; color: var(--gray-500);">Erro ao carregar produtos</td>
                        </tr>
                    `;
                }
            } catch (error) {
        ...
        async function criarProduto(data) {
            ...
            if (result.success) {
                ...
                showToast('❌ Erro ao criar produto: ' + result.error, 'error');
            }
            return result;
        }
        ...
        async function excluirProduto(id) {
            if (confirm('Tem certeza que deseja excluir este produto?')) {
                ...
                if (result.success) {
                    ...
                    showToast('❌ Erro ao excluir produto: ' + result.error, 'error');
                }
            }
        }
        ...
        async function salvarProduto() {
            ...
            const data = {
                ...
                preco_venda: parseFloat(document.getElementById('produto-preco').value) || 0,
            ...
            if (result.success) {
                ...
                showToast('❌ Erro ao salvar produto: ' + result.error, 'error');
            }
        }
        ...
        async function loadPedidos() {
            ...
            try {
                const result = await apiRequest('pedidos', 'GET', null, null, { status });
        ...
        async function atualizarStatusPedido(id, status) {
            const result = await apiRequest('pedido', 'PUT', { status }, id);
            if (result.success) {
        ...
        async function loadClientes() {
            ...
            try {
                ...
                document.getElementById('tabela-clientes').innerHTML = `
                    <tr>
                        <td colspan="7" style="text-align: center; color: var(--danger);">Erro ao carregar clientes</td>
                    </tr>
                `;
            }
        }
        ...
        async function verCliente(id) {
            ...
            if (result.success && result.data) {
                ...
                alert(`👤 Dados do Cliente\n\n` +
                      ...
                      `Cadastro: ${new Date(cliente.data_cadastro).toLocaleDateString('pt-BR')}\n\n` +
                      `📌 Endereços:\n${enderecosStr}`);
            } else {
        ...
        async function loadCategorias() {
            ...
            try {
                ...
                document.getElementById('tabela-categorias').innerHTML = `
                    <tr>
                        <td colspan="6" style="text-align: center; color: var(--danger);">Erro ao carregar categorias</td>
                    </tr>
                `;
            }
        }
        ...
        async function salvarCategoria() {
            ...
            if (result.success) {
                ...
                showToast('❌ Erro ao criar categoria: ' + result.error, 'error');
            }
        }

        async function excluirCategoria(id) {
            if (confirm('Tem certeza que deseja excluir esta categoria?')) {
                // Nota: A API ainda não tem DELETE para categorias, mas implementamos para futura expansão
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    result.data.forEach(cupom => {
                        ...
                        const validade = `${new Date(cupom.data_inicio).toLocaleDateString('pt-BR')} - ${new Date(cupom.data_fim).toLocaleDateString('pt-BR')}`;
                        ...
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>${descontoStr}</td>
                                <td>${validade}</td>
                                <td>${cupom.uso_atual || 0}/${cupom.uso_total || 1}</td>
                                <td><span class="status-badge ${statusClass}">${statusLabel}</span></td>
                                <td>
                ...
                document.getElementById('tabela-cupons').innerHTML = `
                    <tr>
                        <td colspan="7" style="text-align: center; color: var(--danger);">Erro ao carregar cupons</td>
                    </tr>
                `;
            }
        }
        ...
        async function salvarCupom() {
            const data = {
                ...
                valor_desconto: parseFloat(document.getElementById('cupom-valor').value) || 0,
            ...
            if (result.success) {
                ...
                showToast('❌ Erro ao criar cupom: ' + result.error, 'error');
            }
        }
        ...
        function salvarConfiguracoes() {
            const config = {
                ...
                frete_padrao: parseFloat(document.getElementById('config-frete').value) || 0,
